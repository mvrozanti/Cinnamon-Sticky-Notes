...
NoteBox.prototype = {
    ...
    initializeNotes: function() {
        ...
        for ( let noteInfo of settings.getValue("storedNotes") ) {
            ...
            this.notes[i].text.connect("key-press-event", Lang.bind(this.notes[i], function(action,event){
                if(event.get_key_symbol() == Clutter.KEY_Escape){
                    //noteBox.unpinNotes();
                    noteBox.hideNotes();
                    //unfocusText(noteBox.notes[0]);
                    noteBox.notes[i].unfocusText();
                    noteBox.lowerNotes();
                    noteBox.unpinNotes();
                    noteBox.hideNotes();
                    noteBox.update(null,null);
                    //noteBox.menu.toggle();
                }
            }));
            i +=1;
...
MyApplet.prototype = {
    ...
    _init: function(metadata, orientation, panelHeight, instanceId) {
        ...
        uuid = metadata.uuid
        Applet.TextIconApplet.prototype._init.call(this, this.orientation, panelHeight, instanceId);
        // l10n/translation
    ...
    _updateKeybinding: function(){
        global.log("kek")
        Main.keybindingManager.addHotKey("overlay-key-" + this.instanceId, this.overlayKey, Lang.bind(this,function() {
            if(settings.displayState != DisplayState.RAISED || settings.displayState != DisplayState.PINNED) {
                noteBox.initializeNotes();
                noteBox.lowerNotes();
                noteBox.raiseNotes();
                this.menu.toggle();
                focusText(noteBox.notes[0].textBox);
                this.menu.toggle();
            }
        }));
    },
    ...
    on_applet_removed_from_panel: function() {
        Main.keybindingManager.removeHotKey("overlay-key-" + this.instanceId);
        if ( settings.displayState == DisplayState.RAISED || settings.displayState == DisplayState.PINNED ) {
            noteBox.lowerNotes();
        }
        noteBox.destroy();
